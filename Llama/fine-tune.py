...
def compute_metrics(eval_pred):
    ...
    try:
        ...
        if probs.shape[1] == 2:
            ...
            auc = roc_auc_score(
                labels,
                logits[:, 1] if logits.shape[1] > 1 else logits.flatten(),
            )
    except Exception:
        auc = 0.0

    with open("validation_results.csv", "a", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(
            [
...
control_downsampled_fhr = downsample_4hz_to_half_hz(
    control_data.item()["fhr_segments"]
)
control_downsampled_toco = downsample_4hz_to_half_hz(
    control_data.item()["toco_segments"]
)

adverse_downsampled_fhr = downsample_4hz_to_half_hz(
    adverse_data.item()["fhr_segments"]
)
adverse_downsampled_toco = downsample_4hz_to_half_hz(
    adverse_data.item()["toco_segments"]
)

adverse_downsampled_fhr_val = downsample_4hz_to_half_hz(
    adverse_data_val.item()["fhr_segments"]
)
control_downsampled_fhr_val = downsample_4hz_to_half_hz(
    control_data_val.item()["fhr_segments"]
)

adverse_downsampled_toco_val = downsample_4hz_to_half_hz(
    adverse_data_val.item()["toco_segments"]
)
control_downsampled_toco_val = downsample_4hz_to_half_hz(
    control_data_val.item()["toco_segments"]
)


data_records = []

for fhr, toco in zip(control_downsampled_fhr, control_downsampled_toco):
    fhr = remove_trailing_nans(fhr)
    toco = remove_trailing_nans(toco)
    fhr = np.nan_to_num(fhr, nan=0)
    toco = np.nan_to_num(toco, nan=0)

    text_fhr = ",".join(map(str, fhr.astype(np.int16)))
    text_toco = ",".join(map(str, toco.astype(np.int16)))
    ...
    data_records.append({"text": full_text, "label": 0})

for fhr, toco in zip(adverse_downsampled_fhr, adverse_downsampled_toco):
    fhr = remove_trailing_nans(fhr)
    toco = remove_trailing_nans(toco)
    fhr = np.nan_to_num(fhr, nan=0)
    toco = np.nan_to_num(toco, nan=0)
    ...
    data_records.append({"text": full_text, "label": 1})


data_records_val = []

for fhr, toco in zip(adverse_downsampled_fhr_val, adverse_downsampled_toco_val):
    fhr = remove_trailing_nans(fhr)
    toco = remove_trailing_nans(toco)
    fhr = np.nan_to_num(fhr, nan=0)
    toco = np.nan_to_num(toco, nan=0)
    ...
    data_records_val.append({"text": full_text, "label": 1})

for fhr, toco in zip(control_downsampled_fhr_val, control_downsampled_toco_val):
    fhr = remove_trailing_nans(fhr)
    toco = remove_trailing_nans(toco)
    fhr = np.nan_to_num(fhr, nan=0)
    toco = np.nan_to_num(toco, nan=0)
    ...
    data_records_val.append({"text": full_text, "label": 0})
